...
public class LinkedList<T> {
    ...
    public void print() {
        if (isEmpty()) {
            ...
            Node aux;
            aux = getHead();
            String show_complete = "Cedula, Carnet, Nombre, Apellido, Quiz #1, Parcial #1, Proyecto #1, Quiz #2, Parcial #2, Proyecto #2, Total\n";
            for (int i = 0; i < size(); i++) {
                Alumno data = (Alumno) aux.getData();
                show_complete += data.getCedula()+", "+ data.getCarnet() +", "+ data.getNombre() +", "+ data.getApellido()+", "+ data.getQuiz1() +", "+ data.getParcial1() +", "+ data.getProyecto1() +", "+ data.getQuiz2() +", "+ data.getParcial2() +", "+ data.getProyecto2() +" / TOTAL: "+data.getTotal()+ "\n";
                aux = aux.getNext();
            }
            JOptionPane.showMessageDialog(null, show_complete);
        }
    }
    ...
    public void sortList(int op) {
        ...
        while (current != null) {
            ...
            while (next != null) {
                ...
                if (Node.comparator(current, next, op) > 0) {
    ...
    */
    ...
    public Node getNode(int index) { //retorna nodo dado el index
    ...
    public Node next(Node<T> position) { //devuelve el siguiente nodo en caso de que exista
...
//            JOptionPane.showMessageDialog(null, "lista vacia");
            ...
            for (aux = head; null != aux; aux = aux.getNext()) {
                Alumno data = (Alumno) aux.getData();
                //System.out.println(data.getCedula());
                if (((Integer)buscado) == data.getCedula()) {
                    return aux;
                }
            }
        }
